...
冒泡排序是将整个代排序数组进行处理，从头开始进行两两比较，假如前一个元素大于后一个元素的话便进行交换，每次保证较大的元素放置后面的位置，这样一直比较交换到数组末尾，此时数组末尾元素保存的便是整个数组中最大的元素了，也就是说**每一次冒泡排序都将一个元素保存在最终位置上**。之后继续进行元素的比较和交换，将整个数组中第二大的元素找到，放置到最终位置上。对于冒泡排序来说，当某一次的遍历过程中并没有发生元素的交换的话便证明当前的数组已经有序了，便可以提前结束排序操作。
...
在最后的 merge 操作中为了更好的将两个有序数组合并为一个有序数组这里需要使用到一个和待排序数组一样长度大小的辅助数组。
...
对于数组中存在大量和 partition 基准元素相同的数据的情况下，按照之前提出的 partition 实现方式来看的话会导致 partition 分割的数组左右大小不一致。这样就会导致算法的性能退化到 O(n^2) 的情况。因此得对具有大量相同元素的情况进行优化
...
最小生成树对应的数据结构是**带权图**，因此首先来定义下带权图：

#### 带权图

带权图在无权图的基础上在对应的顶点之间的边上增加了权值属性，因此对于邻接矩阵或者邻接表来说，所存储的就不仅仅是简单的有没有边，而是应该存储一个边的全部信息（包含边的权值），因此首先可以创建一个表示边的类：Edge
...
有了 Edge 的抽象类之后就可以来实现稀疏图以及稠密图对应的有权图实现：
...
**最小生成树（Minimum Span Tree）**的定义是：在具有 v 个顶点的图中寻找 v-1 条边，使得这 v-1 条边连接到了所有顶点，并且边上的权值相加最小。

![25](/assets/images/data_structure/25.png)

根据最小生成树的定义可以知道，最小生成树对图的要求是：

- 带权无向图
- 连通图

最小生成树的特点决定了其可以应用在很多的现实问题上：

- 电缆布线设计
- 网络设计
- 电路设计

为了解决最小生成树问题，人们提出了两个主要的算法。这两个算法都是基于一个非常重要的定理：**切分定理（Cut Property）**

下面介绍一些相关的基本概念：

**切分**：把图中的节点分为两部分，成为一个切分（Cut）。

**横切边**：如果一个边的两个端点属于切分不同的两边，这个边就称为横切边（Crossing Edge）

![26](/assets/images/data_structure/26.png)

接下来介绍切分定理：

给定**任意**切分，横切边中权值最小的边必然属于最小生成树。

![27](/assets/images/data_structure/27.png)

根据这个定理我们便可以去学习基于这个定理的最小生成树算法。

#### 普里姆算法

普里姆算法（Prim）是求解无向带权图最小生成树的一个算法，根据其算法的运行流程，可以将其表示为：**加点法**。

普里姆算法有两种实现方式，下面首先介绍第一种：

##### Lazy Prim

lazy prim 算法基于切分定理，首先将起始节点作为一部分与图中其他节点进行切分。然后将起始节点周围与其他节点的边保存到**最小堆**中，这是因为最小堆的性质可以保证保存进堆中元素的有序性。然后当堆不为空的时候开始执行循环操作：首选取出堆顶边，判断当前边所连接的顶点是否已经被访问过了，假如没有被访问过的话就将该顶点标记为访问并且将该顶点所对应的边保存到最小堆中，此时切分的部分发生了变化。加下来继续执行取边、访问节点、加边的操作。直到最后最小堆为空。

只所以说这种实现方式的 prim 为 lazy prim 就在于每次会将无效边继续保存在堆中，也就是将所有的边都保存在堆中了，然后一一出堆，并没有对加入堆中的边进行相应的操作。

为了实现 lazy prim 首先得建立一个最小堆。
...
Lazy prim 的时间复杂度为：O(ElogE)：E为边的数量

而我们其实可以对上面的算法进行优化，使其的时间复杂度降低为：O(ElogV)

##### Prim

上面实现的 lazy prim 算法中最浪费时间的地方是将一些不是横切边的边也加入了最小堆中。其实对于 prim 算法来说，是不需要保存所有的边到堆中的，并且保存进堆中的边应该是每个节点之间的最小横切边。

现在我们需要保存**每个节点之间相连的最短横切边**，并且这些横切边是需要更新的，会随着节点的不同切分而产生更小的横切边，也就是保存的数据还得进行更新。这里就需要用到**最小索引堆（indexMinHeap）**。

我们在最小索引堆中保存的索引就是每一个点，而堆中保存的数据就是每个点距离当前已经切分的图之间最小的横切边，这样就可以优化的实现prim算法。
...
克鲁斯卡尔算法（Kruskal）是另一种可以求解无向加权图中最小生成树的算法。他的主要思想是：

> 首先将图中所有的边按照权值大小进行排序，然后按照排序结果从小到大的依次将边加入到最小生成树中，前提是假如的边并不会导致当前的最小生成树中出现环。知道所有的顶点都被并入到最小生成树中，此时便已经找到了该图所对应的最小生成树。

其实克鲁斯卡尔算法的思路非常简单，因此非常适合于**手动寻找最小生成树的过程中**。其中只涉及到一个重点需要解决的问题：如何判断所要加入的边会在最小生成树中形成环。这里可以使用到之前介绍到的一种数据结构：**并查集**

使用 UnionFind 可以快速判断当前最小生成树中是否有环：每次加入节点的时候设置当前节点的根，当有新边需要并入树中节点的时候只需要判断两个节点是否具有相同的根节点，加入根节点相同则表示加入这条边会形成环。
...
本章介绍了三种不同的求解最小生成树的算法，下面介绍每种算法所对应的时间复杂度：

- Lazy prim：O(ElogE)
- Prim：O(ElogV)
- Kruskal：O(ElogE)

其中，目前为止最优的时间复杂度为O(ElogV)。

上面实现的最小生成树算法都借助到了一些高级的数据结构：**最小索引堆、并查集**。

对于图中假如存在相同的横切边的话，使用上面算法求出的最小生成树是**不唯一**的。

### 5.最短路径

#### 5.1 最短路径问题介绍

最短路径（Shortest Path）问题是求解从一个节点到其余节点的最短路径。常见的应用：

- 路径规划
- 工作任务规划

之前使用**广度优先遍历**算法在一颗无向无权图中的遍历结果就是一颗**最短路径树（Shortest Path Tree）**，所求得的路径是属于：**单源最短路径（Single Source Shortest Path）**。

而本章主要探讨的是**有向带权图**的最短路径。

**松弛操作（Relaxation）**：从一颗有权图中的寻找从一个节点到另一个节点是否存在绕道之后权值更小的路径。

#### 5.2 Dijkstra 单源最短路径算法

前提：图中不能有负权边，不过日常生活中的问题都不会出现负权边。

时间复杂度：O(ElogV) 通过最小索引堆来实现。

算法思想：

> 从起始节点开始，然后将该节点可以到达的所有顶点中选择一个路径最短的节点并入，然后执行 Relaxation 方法，寻找更短的路径。然后从当前可以到达的节点中选择未被访问的节点中权值最小的并入到当前以访问的节点中，然后继续执行 Relaxation操作，直到所有的节点都被访问。
...
#### 5.3 Bellman-Ford 单源最短路径算法

![28](/assets/images/data_structure/28.png)

如图所示，假如在一个图中存在负权边的话那么每添加一个节点都要对之前所添加的所有节点都执行 Relaxation 操作。而一旦产生了负权环，那么这个图中就不存在最短路径。因为负权边会产生负权环，一直在该环中循环的权值会小于到其他的节点的权值。

因此下面介绍一种可以用来处理图中包含负权边的方法：Bellman-Ford 算法，但是使用该算法的前提是：**图中不存在负权环**。

但是 Bellman-Ford 可以判断图中是否有负权环。判断的依据在于：

> 如果一个图中没有负权环，那么从一点到另一点的最短路径，最多经过所有的V个顶点，有V-1条边。否则，存在顶点经过了两次，即存在负权环。

时间复杂度:O(EV)

算法思想：

> 从起始节点开始，依次选择当前权值最小且未被访问过的节点进行访问，然后进行第一次的 Relaxation 操作。之后在对所有的节点进行第二次的 Relaxation 操作。
>
> 对所有的点进行V-1次松弛操作，理论上就找到了从源点到其他所有节点的最短路径。
>
> 如果还可以继续松弛，说明原图中有负权环。
...
1、单源最短路径：distTo[i]初始化

2、Bellman-Ford 算法的优化：利用队列数据结构 queue-based bellman-ford 算法。
...
| Bellman-Ford | 无负权边       | 有向图         | O(VE)      |
| 利用拓扑排序 | 有向无权图 DAG | 有向图         | o(V+E)     |

4、所有对最短路径算法

上面介绍的都是一些单源最短路径算法，对于处理所有对的最短路径算法来说，除了多次运行单源最短路径之后还有一种算法：Floyed 算法，可以处理无负权环的图，时间复杂度为：O(V^3)
